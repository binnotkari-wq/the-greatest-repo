...
#Obtention du nom de la sauvegarde existante la plus récente dans le cas de la sauvegarde incrémentielle (ou de la sauvegarde initiale dans le cas d'un différentiel). On stocke en variable uniquement la partie datée du nom (grâce à cut) du dernier répertoire (ou du premier pour le differentiel) de sauvegarde existant (grâce à ls -dr qui ne listera que les répertoires qui nous interressent) puis head (qui fait un tri en gardant le nom le plus "haut" donc dans ce cas de figure le nom qui contiendra la date la plus récente) . Cette variable sera passée à rsync pour lui indiquer le chemin de la dernière (ou première!) sauvegarde à comparer. 
...
#OPT='-auv'
#ORIGINE=/media/benoit/1To/Benoit/
